...
fun init (otw: CAP_HERO, ctx: &mut TxContext) {
    ...
    factory.add_permission<Hero>(type_name::with_defining_ids<Mint>());
    factory.add_permission<Hero>(type_name::with_defining_ids<LevelUp>());  
    factory.add_permission<Hero>(type_name::with_defining_ids<IncreaseHP>());
    factory.add_permission<Hero>(type_name::with_defining_ids<IncreaseAttack>());
    factory.add_permission<Hero>(type_name::with_defining_ids<IncreaseDefense>());
    factory.add_permission<Hero>(type_name::with_defining_ids<IncreaseSpeed>());
    factory.add_permission<Hero>(type_name::with_defining_ids<Burn>());
...
public fun level_up(self: &mut Hero, _cap: &Capability<LevelUp>) {
    self.level = self.level + 1;
}

public fun increase_hp(self: &mut Hero, _cap: &Capability<IncreaseHP>) {
    self.hp = self.hp + 10;
}

public fun increase_attack(self: &mut Hero, _cap: &Capability<IncreaseAttack>) {
    self.attack = self.attack + 1;
}

public fun increase_defense(self: &mut Hero, _cap: &Capability<IncreaseDefense>) {
    self.defense = self.defense + 1;
}

public fun increase_speed(self: &mut Hero, _cap: &Capability<IncreaseSpeed>) {
    self.speed = self.speed + 1;
}

public fun burn(self: Hero, _cap: &Capability<Burn>) {
